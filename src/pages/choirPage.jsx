...
const ChoirCard = React.memo(({ choir, onOpenSongs, index }) => {
  return (
    <div
      ...
      <div className="relative z-10">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {choir.committee.slice(0, 3).map((member, idx) => (
              <span
                key={idx}
        ...
        <div className="flex justify-between items-center pt-5 border-t border-gray-100">
          ...
          >
            <span className="relative z-10 flex items-center gap-2">
              ...
              View Songs ({choir.songs?.length || 0})
            </span>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 max-w-md mx-auto">
                This choir hasn't uploaded any songs yet. Check back soon for
                updates!
              </p>
            </div>
          )}
...
export default function ChoirsPage() {
    ...
    async (choir) => {
      ...
      try {
        const thumbs = await Promise.all(
          choir.songs.map(async (song, index) => {
            try {
  ...
  const filteredChoirs = useMemo(() => {
    ...
    return choirs.filter(
      (choir) =>
        ...
        choir.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
        choir.president.toLowerCase().includes(searchTerm.toLowerCase()) ||
        choir.committee.some((member) =>
  ...
  const sortedChoirs = useMemo(() => {
    return [...filteredChoirs].sort((a, b) => a.name.localeCompare(b.name));
  }, [filteredChoirs]);
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-gray-900">
                    {choirs.reduce(
                      (acc, choir) => acc + (choir.songs?.length || 0),
                      0
                    )}
                  </p>
                ...
                <div>
                  <p className="text-3xl font-bold text-gray-900">
                    {choirs
                      ...
                      .reduce((acc, choir) => acc + choir.committee.length, 0)}
                  </p>
