...
const AdminDepartments = () => {
  ...
  const handleEdit = (dept) => {
    ...
    setFormData({
      ...
      committee: (dept.committee || []).map((c) => `${c.role}: ${c.name}`).join("\n"),
      ...
      <div className="bg-white p-6 rounded-2xl shadow max-w-5xl mx-auto mb-10">
        ...
        <div className="flex border-b mb-6">
          {["info", "members", "plans"].map((tab) => (
            ...
            >
              {tab === "info" ? "Basic Info" : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
          ...
          >
            <div>
              ...
              {dept.members && dept.members.length > 0 && (
                <p className="mb-1 text-gray-600">Members: {dept.members.join(", ")}</p>
              )}
              {dept.committee && dept.committee.length > 0 && (
                <p className="mb-1 text-gray-600">
                  Committee: {dept.committee.map((c) => `${c.role}: ${c.name}`).join(", ")}
                </p>
              )}
              {dept.plans && dept.plans.length > 0 && (
                <p className="mb-1 text-gray-600">Plans: {dept.plans.join(", ")}</p>
              )}
              {dept.actions && dept.actions.length > 0 && (
                <p className="mb-2 text-gray-600">Actions: {dept.actions.join(", ")}</p>
              )}
            </div>
